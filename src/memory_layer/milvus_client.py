...
class SemanticVectorStore:
    """
    Manages the semantic memory of the traffic analytics agent using Milvus Lite.
    Stores text descriptions generated by the VLM and handles temporal chunking.
    """
    def __init__(self, db_path: str = "data/milvus_storage/semantic_memory.db", collection_name: str = "traffic_events"):
        # Ensure the data directory exists
        Path(db_path).parent.mkdir(parents=True, exist_ok=True)
        
        # Initialize Milvus Lite client (runs locally in-process)
        self.client = MilvusClient(db_path)
        self.collection_name = collection_name
        
        # Load a lightweight, fast embedding model optimized for edge devices
        print("Loading SentenceTransformer embedding model...")
        self.embedding_model = SentenceTransformer('all-MiniLM-L6-v2')
        self.dim = 384  # Output dimension for all-MiniLM-L6-v2
    ...
    def _initialize_collection(self):
        ...
        if not self.client.has_collection(self.collection_name):
            print(f"Creating new Milvus collection: {self.collection_name}")
            # Create collection with auto-id enabled and defined vector dimensions
