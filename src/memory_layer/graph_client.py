...
class GraphClient:
    ...
    def query_graph(self, cypher_query: str) -> List[Dict[str, Any]]:
        ...
        """
        # Fix 2: only allow read queries — reject any LLM-generated query that
        # contains writes (CREATE, DELETE, MERGE, SET, REMOVE) to prevent graph corruption.
        if not cypher_query.strip().upper().startswith("MATCH"):
            return [{"error": "Only read queries starting with MATCH are permitted."}]
        try:
            # Execute the query
